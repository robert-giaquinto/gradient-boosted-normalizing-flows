...
class BoostedVAE(VAE):
	...
	def __init__(self, args):
		...
		self.num_learners = args.num_learners
		self.num_flows = args.num_flows
		self.learner = 0  # current learner being trained
	...
	def encode(self, x):
		...
		# return amortized u an w for first c flows
		u, w, b = [], [], []
	...
	def forward(self, x):
		...
		# ideally, would sample a different flow for each observation. instead, for efficient just sample a flow PER BATCH
		c = torch.multinomial(self.rho[0:(self.learner+1)], 1, replacement=True).item()

		for k in range(self.num_flows):
			# TODO FIX LDJ TERM
			z_ck, ldj = self.flow(z[k], u[c][:, k, :, :], w[c][:, k, :, :], b[c][:, k, :, :])
			z.append(z_ck)
			self.log_det_j += ldj
